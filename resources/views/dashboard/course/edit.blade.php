                                        ...
                                        <div class="form-group {{($errors->has('section_id')) ? 'has-error' : ''}}">
                                            ...
                                            <select name="section_id" class="form-control" id="section_id" type="text">
                                                ...
                                                @forelse($sections as $section)
                                                    <option
                                                        {{ old('section_id', $course->section_id) == $section->id ? "selected" : "" }} value="{{ $section->id }}">{{ $section->classes->name .' / '.$section->name }}</option>
                                                @empty
                                        ...
                                        <div class="form-group {{($errors->has('course_type_id')) ? 'has-error' : ''}}">
                                            ...
                                            <select name="course_type_id" class="form-control" id="course_type_id"
                                                ...
                                                @forelse($courseTypes as $type)
                                                    <option
                                                        {{ old('course_type_id', $course->course_type_id) == $type->id ? "selected" : "" }} value="{{ $type->id }}">{{ $type->name }}</option>
                                                @empty
                                                    <option>No data</option>
                                                @endforelse
                                            </select>
                                    ...
                                    <div class="form-group {{($errors->has('schedule_id')) ? 'has-error' : ''}}">
                                        ...
                                        <select multiple name="schedule_id[]" class="form-control" id="schedule_id"
                                            ...
                                            @forelse($schedules as $schedule)
                                                <option
                                                    {{ in_array($schedule->id,old('schedule_id', $course->schedules->pluck('id')->toArray()) ?: []) ? "selected" : "" }} value="{{ $schedule->id }}">{{ $schedule->day. '/' . $schedule->start. '/' . $schedule->end }}</option>
                                            @empty
                                                <option>No data</option>
                                            @endforelse
                                        </select>
                                    ...
                                    <div class="form-group {{($errors->has('teacher')) ? 'has-error' : ''}}">
                                        ...
                                        <select multiple name="teacher[]" class="form-control" id="teacher" type="text">
                                            ...
                                            @forelse($teachers as $teacher)
                                                <option
                                                    {{ in_array($teacher->id, old('teacher', $course->teacher->pluck('id')->toArray()) ?: []) ? "selected" : "" }} value="{{ $teacher->id }}">{{ $teacher->fullname }}</option>
                                            @empty
                                                <option>No data</option>
                                            @endforelse
                                        </select>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group {{($errors->has('body')) ? 'has-error' : ''}}">
                                        ...
                                        <textarea cols="5" rows="5" name="body" class="form-control" id="body"
                                                  type="text"
                                                  placeholder="enter course description">{{ old('body', $course->body) }}</textarea>
                                    </div>
                                </div>
                            </div>
